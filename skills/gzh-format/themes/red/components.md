...
> **使用说明**：本组件库为「红色」主题（经典编辑风），所有组件使用**内联样式**，可直接复制粘贴到微信公众号编辑器。
>
> **设计风格**：红色克制点睛。淡粉下划线为主标记、左竖条块引用、红色仅在锚点处出现。编号章节 + 引言卡 + 签名区的经典编辑骨架，适合观点、深度分析、盘点、教程、随笔类文章。
>
> **公众号平台限制须知**：
...
> - 卡片本身已经表达引言语义,不再添加大号或首尾装饰引号;多行内容只保留单次换行,不插空白段落
...
<section style="margin:10px 10px 32px;background:#ffffff;border-radius:12px;box-shadow:0 4px 24px -4px rgba(220,38,38,0.15);padding:28px 24px 22px;overflow:hidden;">
  <p style="font-size:16px;font-weight:800;color:#1C1917;margin:0 0 8px;line-height:1.75;padding-left:4px;">
...
多行代码块用通用增量库 `themes/_shared/common-components.md` 的 1a 深色 / 1b 浅色（左竖条换 `#DC2626`），禁 `white-space:pre`。
...
|---|---|---|
| `# 标题` | 不使用 | 公众号文章标题在平台设置 |
| 文章开头 `> 引言金句` | 组件 2 白底红色光晕引言卡 | 视角与外标题错开;无装饰引号,多行紧凑换行 |
