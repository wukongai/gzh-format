...
def main():
    ...
    refs = sorted(glob.glob(os.path.join(root, "themes", "**", "*.md"), recursive=True))
    if not refs:
        print(f"未找到 {root}/themes/**/*.md")
        sys.exit(1)

    total_err = total_warn = clean = 0
    print(f"📐 组件库源头检查：{len(refs)} 个主题/共用库\n")
    for path in refs:
        name, found = lint_file(root, path)
        if not found:
            clean += 1
            continue
