...
> **使用说明**：本组件库为「简约」主题，所有组件使用**内联样式**，可直接复制粘贴到微信公众号编辑器。
>
> **设计风格**：内刊/编辑部手记质感——白色全局底 + 墨黑强调 + 轻橄榄灰边框，橙色只作克制点睛，陶土棕作次强调/进行中状态色。信息密度偏高但不堆装饰，适合系统性说明文档、深度评测、案例复盘一类内容，气质克制、理性、略带"内部资料"的严肃感。
>
> **公众号平台限制须知**：
...
次要文字：                     #65675e
弱化文字：                     #9ea096
边框/分隔线：                   #dfe2d8（普通边框） / #bfc1b7（强调边框）
全局背景：                     #ffffff（不要给整篇文章加米黄/纸感底）
局部米白卡片底：                 #fdfdf8（仅用于少量卡片/图片框）
浅橄榄灰背景：                   #eeefe9
标签浅底：                     #e5e7e0
强调橙（显式下划线/前导词/CTA）： #ed7b2f
...
**显式下划线**：默认不主动给正文关键词加下划线。只有源文显式使用 `<u>` / `++...++` 时,才用 `border-bottom:2px solid #ed7b2f;font-weight:600;color:#23251d;`。
...
```html
<section style="max-width:677px;margin:0 auto;padding:8px;box-sizing:border-box;background:#ffffff;color:#4d4f46;font-family:'IBM Plex Sans',-apple-system,system-ui,'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif;line-height:1.75;">
...
<section style="background:#fdfdf8;border:1px solid #bfc1b7;border-radius:6px;overflow:hidden;font-family:'IBM Plex Sans',-apple-system,system-ui,sans-serif;">
  <section style="padding:28px 24px 22px;">
    ...
    <section style="display:flex;align-items:stretch;gap:18px;">
      ...
      <section style="flex-shrink:0;width:112px;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#eeefe9;border:1px solid #dfe2d8;border-radius:6px;padding:8px;">
...
**用途**：大章节分隔，默认编号形式 01/02/03…，数字在上、标题在下。章节之间靠留白拉开节奏,不加 `PART`、英文标签、竖线或底部分隔线。

**可替换字段**：`{{编号}}` `{{标题}}`

```html
<section style="margin-top:56px;margin-bottom:20px;">
  <section style="font-family:'PingFang SC','Hiragino Sans GB','Microsoft YaHei',-apple-system,BlinkMacSystemFont,sans-serif;text-align:left;">
    <p style="margin:0 0 10px;font-family:'Helvetica Neue',Arial,'PingFang SC','Hiragino Sans GB','Microsoft YaHei',-apple-system,BlinkMacSystemFont,sans-serif;font-size:54px;font-weight:900;color:#23251d;line-height:0.95;letter-spacing:0.5px;"><span leaf="">{{编号}}</span></p>
    <p style="margin:0;font-size:22px;font-weight:800;color:#23251d;line-height:1.35;letter-spacing:0;text-align:left;"><span leaf="">{{标题}}</span></p>
  </section>
</section>
```
...
**用途**：带前导词（Kicker）+ 进度指示的标题，适合章节内的小分节。默认不加底线,避免和正文产生重复装饰。
...
<section style="margin-top:24px;">
  <section style="font-family:'IBM Plex Sans',-apple-system,sans-serif;">
    <section style="display:flex;align-items:flex-end;gap:12px;flex-wrap:wrap;">
...
## 组件 9A 章节内轻量小标题 micro-heading

**用途**：章节内的二级条目标题。用于源文没有写 `###`，但段落语义明显是小标题的短句，例如 `第一，什么时候触发？`、`第二，默认怎么走？`、`第一层是主题索引。`。这类内容不应按普通正文渲染，否则层级太弱；也不要升成章节标题，避免打断 H2 的主结构。

**自动识别规则**：默认 renderer 会把短段落中以下模式映射为本组件：
...
```html
<section style="margin:26px 0 10px;padding:0 8px;">
  <h4 style="margin:0;font-size:16px;font-weight:800;color:#23251d;line-height:1.65;letter-spacing:0;">
    <span leaf="">{{小标题}}</span>
  </h4>
</section>
```
...
## 组件 9B 非数字序列 example-sequence

**用途**：章节内连续举例或动作步骤，但源文没有显式写成 Markdown 列表时使用。典型模式是多段连续 `比如...`、`例如...`，或在 `理想动作应该是：` 后接的多条短动作句。它们语义上是“序列项”，不是普通正文段落。

**自动识别规则**：默认 renderer 只会把连续 2 条以上的以下短段落映射为本组件，并用短横线标记。单个 `比如...` / `例如...` / 短动作句仍按普通正文段落渲染，避免一条解释句被过度列表化。
...
```html
<section style="margin:8px 0 18px;padding:0 8px;">
  <section style="font-family:'IBM Plex Sans',-apple-system,sans-serif;">
    <section style="display:flex;align-items:flex-start;margin-bottom:6px;">
      <span style="display:inline-block;color:#ed7b2f;font-size:13px;font-weight:800;line-height:1.65;margin-right:9px;vertical-align:top;flex-shrink:0;"><span leaf="">-</span></span>
      <p style="font-size:13px;color:#4d4f46;margin:0;line-height:1.65;flex:1;text-align:justify;"><span leaf="">{{序列项}}</span></p>
    </section>
  </section>
</section>
```
...
**c. 橙色下划线** —— 仅用于源文显式 `<u>` / `++...++` 的短语,不主动给普通正文关键词添加
...
## 组件 13 短横线列表 hyphen-list-compact

**用途**：简单并列条目，不需要标题+说明两层结构时用它（需要两层结构用组件 24）。标记用轻量短横线,字号和行距都比正文稍紧,避免列表看起来像一堆按钮。

**可替换字段**：`{{列表项}}`（可重复 N 次）

```html
<section style="margin-top:16px;margin-bottom:16px;padding:0 8px;">
  <section style="font-family:'IBM Plex Sans',-apple-system,sans-serif;">
    <section style="display:flex;align-items:flex-start;margin-bottom:4px;">
      <span style="display:inline-block;color:#1e1f23;opacity:0.68;font-size:12px;font-weight:500;line-height:1.55;margin-right:9px;vertical-align:top;flex-shrink:0;"><span leaf="">-</span></span>
      <p style="font-size:13px;color:#4d4f46;margin:0;line-height:1.55;flex:1;text-align:justify;"><span leaf="">{{列表项}}</span></p>
    </section>
  </section>
</section>
```
...
## 组件 14 轻量引言 intro-quote

**用途**：文章开头 `>` 引言或轻量旁白。只用浅灰底 + 小字建立引言层级,不做深色标题条、封面卡或装饰性引号。

**换行契约**：

- 容器本身已经表达“引言”语义,正文首尾不再加中文双引号。
- 多行引言允许换行,相邻非空行之间只输出一个 `<br>`。
- Markdown 引用里的空白行必须折叠,不得渲染成 `<br><br>`。

**可替换字段**：`{{引言第一行}}` `{{引言第二行（可选）}}`

```html
<section style="margin-top:4px;margin-bottom:24px;padding:0 8px;">
  <section style="background:#f1f2ee;border-radius:8px;padding:14px 16px;font-family:'IBM Plex Sans',-apple-system,sans-serif;">
    <p style="margin:0;font-size:13px;line-height:1.75;color:#6b7067;text-align:justify;"><span leaf="">{{引言第一行}}</span><br><span leaf="">{{引言第二行（可选）}}</span></p>
  </section>
</section>
```

只有一行时,删除 `<br>` 和第二个 `<span>`,不保留空行。

---

## 组件 15 重点观点卡 key-point-card

**用途**：段落级重点强调，比正文强调更完整。只用加粗和浅边框建立层级,不主动加下划线。
  ...
  <section style="font-family:'IBM Plex Sans',-apple-system,sans-serif;">
    <section style="background:#fdfdf8;border-radius:6px;padding:16px 18px;border:1px solid #bfc1b7;">
      <p style="font-size:14px;color:#4d4f46;margin:0;line-height:1.8;text-align:justify;">
        <strong style="color:#23251d;font-weight:800;"><span leaf="">{{重点观点}}</span></strong><span leaf="">&nbsp;{{补充说明}}</span>
      </p>
    </section>
  </section>
</section>
...
## 组件 16 分割线 divider-solid（默认禁用）

本项目默认不使用分割线。章节之间靠组件 3 的上下留白建立层级;只有用户明确要求“加分隔线”时才临时设计。

---

## 组件 17 分割点 divider-dots（默认禁用）

本项目默认不使用分割点。它容易在 part 之间重复出现,干扰连续阅读。

---

## 组件 18 正文对齐图片 media-aligned-image

**用途**：无说明文字的正文图片。边框和正文文字左右对齐,边框颜色保持很浅,不做出血、虚线或重阴影。
...
<section style="margin-top:24px;">
  <section style="font-family:'IBM Plex Sans',-apple-system,sans-serif;">
    <section style="background:#fdfdf8;border-radius:8px;padding:4px;border:1px solid #dfe2d8;">
...
<section style="margin-top:24px;">
  <figure style="font-family:'IBM Plex Sans',-apple-system,sans-serif;margin:0;">
    <section style="border-radius:8px;overflow:hidden;border:1px solid #dfe2d8;padding:4px;display:block;">
      <span leaf=""><img src="{{图片URL}}" alt="图片" style="max-width:100%;height:auto;display:block;margin:0 auto;"></span>
    </section>
...
<section style="margin-top:24px;">
  <section style="padding:18px;border-radius:6px;background:#fdfdf8;border:1px solid #bfc1b7;text-align:center;font-family:'IBM Plex Sans',-apple-system,sans-serif;">
    ...
    <section style="min-height:140px;border-radius:6px;background:#eeefe9;display:flex;align-items:center;justify-content:center;border:1px solid #dfe2d8;">
    ...
    <section style="display:flex;">
      ...
      <section style="flex:1;padding-bottom:12px;">
        ...
        <section style="text-align:center;margin-bottom:4px;">
          <span leaf=""><img src="{{图片URL}}" alt="案例配图" style="max-width:100%;height:auto;display:block;margin:0 auto;border-radius:6px;border:1px solid #dfe2d8;"></span>
        </section>
...
**用途**：信息量最大的收尾形式——呼应组件 3 的堆叠标题结构,用 `///` 作为收尾标记,不加 `END`、竖线或底部分隔线。
...
<section style="margin-top:24px;">
  <section style="font-family:'IBM Plex Sans',-apple-system,sans-serif;">
    <section style="margin-bottom:18px;">
      <p style="margin:0 0 8px;font-size:38px;font-weight:800;color:#23251d;line-height:1;letter-spacing:-1px;"><span leaf="">///</span></p>
      <p style="margin:0;font-size:22px;font-weight:800;color:#23251d;line-height:1.35;letter-spacing:0;"><span leaf="">{{结尾标题}}</span></p>
      <p style="margin:8px 0 0;font-size:13px;color:#65675e;line-height:1.7;"><span leaf="">{{结尾说明}}</span></p>
    </section>
    ...
    <section style="background:#eeefe9;border-radius:6px;padding:16px 18px;border:1px solid #bfc1b7;text-align:center;">
      <p style="font-size:15px;color:#23251d;margin:0;line-height:1.6;"><strong style="font-weight:800;"><span leaf="">{{最后总结}}</span></strong></p>
    </section>
  </section>
</section>
```
...
## 组件 35 编号问题清单 question-list-numbered

**用途**：连续追问、问题清单、判断清单。适合文章中连续出现 2 条以上短问句时，把“连续发问”渲染成可扫描的编号块；它不是普通列表，也不是 FAQ 展开区，而是一个推动读者思考的节奏组件。

**自动识别规则**：默认 renderer 会把连续 2 条以上、每条以 `？` / `?` 结尾、长度适中的独立段落映射为本组件。单独一个问句仍按普通正文段落渲染，避免过度设计。

**可替换字段**：`{{编号}}` `{{问题}}`（可重复 N 次）

```html
<section style="margin:0 0 22px;padding:0 8px;">
  <section style="background:#fdfdf8;border:1px solid #bfc1b7;border-radius:6px;padding:4px 16px;font-family:'IBM Plex Sans',-apple-system,sans-serif;">
    <section style="display:flex;align-items:flex-start;gap:10px;padding:12px 0;">
      <span style="display:inline-block;min-width:34px;text-align:center;background:#1e1f23;color:#ffffff;border-radius:4px;padding:2px 0;font-size:11px;line-height:1.6;font-weight:800;flex-shrink:0;"><span leaf="">{{编号}}</span></span>
      <p style="font-size:15px;color:#4d4f46;margin:0;line-height:1.7;flex:1;font-weight:700;text-align:justify;"><span leaf="">{{问题}}</span></p>
    </section>
    <section style="display:flex;align-items:flex-start;gap:10px;padding:12px 0;border-top:1px solid #dfe2d8;">
      <span style="display:inline-block;min-width:34px;text-align:center;background:#1e1f23;color:#ffffff;border-radius:4px;padding:2px 0;font-size:11px;line-height:1.6;font-weight:800;flex-shrink:0;"><span leaf="">{{编号}}</span></span>
      <p style="font-size:15px;color:#4d4f46;margin:0;line-height:1.7;flex:1;font-weight:700;text-align:justify;"><span leaf="">{{问题}}</span></p>
    </section>
  </section>
...
**使用边界**：
- 连续问句组才用；单个问句仍用组件 10 正文段落。
- 只用于真实问题，不用于以问号结尾的标题、引用、图片说明或表格单元格。
- 编号在每组内部从 `01` 重新开始，不继承章节编号。

---

## 多行代码块 → 用通用增量库

本主题不设专属多行代码块组件；Markdown 的三反引号围栏代码块直接用 `themes/_shared/common-components.md` 的 1a 深色代码块（默认）或 1b 浅色代码块，左竖条/强调色换成本主题墨色 `#1e1f23`；行内代码用本主题"文字强调 d"或组件 11。
...
<section style="max-width:677px;margin:0 auto;padding:8px;box-sizing:border-box;background:#ffffff;color:#4d4f46;font-family:'IBM Plex Sans',-apple-system,system-ui,'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif;line-height:1.75;">
  ...
  <!-- 2. 开篇轻量引言（组件14 intro-quote，可选，仅原文有独立引言/背景说明时使用） -->

  <!-- 3. 第一章（组件3 section-title，默认分节样式，编号01） -->
  <!--    章内按需组合：组件10 正文段落(+文字强调a-e) / 组件11 行内代码段落 / 组件12 新旧对照段落 / 组件13 短横线列表 / 组件24 条目列表卡 / 组件35 编号问题清单 -->
...
| **标记层** | 浅底高亮框（文字强调 b）、显式下划线（文字强调 c） | 高亮原文已有强调;下划线只保留源文显式语义 | 按源文出现 |
| **容器层** | 重点观点卡 15、条目列表卡 24、编号问题清单 35、轻量引言 14、各类分节条 4/6/9 | 小结、清单、连续追问、过渡 | 按需 |
...
| 观点/深度分析 | 头图卡2 + 章节标题3 + 强调标题8 + 正文段落10 + 重点观点卡15 + 暗色摘要边框31 | 轻量引言14、新旧对照段落12 |
| 数据复盘/报告 | 头图卡2 + 期号徽章条6 + 前导词标题9 + 图表占位卡20 + 精简对照表22 + 暗色摘要分栏30 | 流程示意图23、行内代码段落11（指标术语） |
| 访谈/人物特稿 | 头图卡2 + 章节标题3 + 轻量引言14 + 正文段落10 + 案例时间线26 + 作者签名条33 | 新旧对照段落12（观点前后变化）、暗色摘要边框31（引语收束） |
| 生活/情感随笔 | 头图卡2 + 内刊标签条4 + 正文段落10 + 重点观点卡15 | 摘要横幅条32 |
...
| 文章开头 `> 引言` | 组件14 轻量引言 | 浅灰底、小字、无装饰引号;多行只保留单次换行,空白引用行折叠 |
| `## 章节标题` | 组件3 section-title（默认）/ 组件6 期号徽章条（专题式，可选变体） | 编号 01/02/03… |
| `### 子标题` | 组件7 步骤内联标题（教程步骤）/ 组件9 前导词标题（系列分节）/ 组件8 强调标题（观点强调） | 按语境三选一 |
| `第一，...？` / `第二，...。` / `第一层是...。` | 组件9A 章节内轻量小标题 | 源文未写 `###` 但语义是章节内二级标题时自动加粗成 H4 |
| 连续 2 条以上 `比如...` / `例如...` / 短动作句 | 组件9B 非数字序列 | 举例和动作步骤成组出现时才属于序列语义;单个解释句仍用普通段落 |
| 连续 2 条以上短问句 | 组件35 编号问题清单 | 连续追问属于问题清单语义,每组内部从 `01` 重新编号;单个问句仍用普通段落 |
| 普通段落 | 组件10 正文段落 | 不主动加关键词下划线 |
| `**加粗文字**` | 文字强调 a 强调加粗 | |
| `==高亮文字==` | 文字强调 b 浅底高亮框 | |
| `<u>下划线</u>` / `++文字++` | 文字强调 c 橙色下划线 | 只保留源文显式下划线语义 |
| `~~删除文字~~` | 文字强调 e 删除线旧词 | 常与新表述搭配用组件12 |
| 行内 `` `code` `` / 技术名词/字段名 | 文字强调 d 行内代码标签 | 整句强调用组件11 |
| ` ``` 多行代码块 ``` ` | 通用库1a深色/1b浅色 | 左竖条换本主题墨色 `#1e1f23` |
| 无序/有序列表 | 组件13 短横线列表（简单）/ 组件24 条目列表卡（需标题+说明两层） | 标记用 `-`,行距更紧 |
| 表格 | 组件22 精简对照表（≥2列数据对照）/ 组件21 对比摘要卡（二元对比） | 不用 `table`/`grid`，flex 仿表格 |
| `![说明](图片)` | 组件19 图片卡 | 有说明才加 `<figcaption>` |
| `![](图片)` 无说明 | 组件18 正文对齐图片 | 不做通栏出血 |
| `---` 分割线 | 跳过 | 本项目默认不输出视觉分隔线 |
| `> 引用`（非开头） | 组件14 轻量引言 / 组件15 重点观点卡 | 视语气选择 |
| 案例/示例展开 | 组件26 案例时间线 | |
| 常见问题 | 组件25 常见问题列表 | |
